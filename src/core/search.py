...
class ParallelWorkers(ListRWTools, SSHTools):
    ...
    def generate_job(self, round, init_pos, init_type, init_symm,
        ...
        for i, node in enumerate(node_assign):
            job = [round, i, node, init_grid[i], init_sg[i]]
    ...
    def sampling_with_ssh(self, atom_pos, atom_type, atom_symm,
                          round, path, node, grid_name, space_group):
        """
        ...
        for i in range(len(atom_pos)):
            option = f'--pos {atom_pos[i]} --type {atom_type[i]} --symm {atom_symm[i]} ' \
                     ...
                     f'--grid {grid_name[i]} --sg {space_group[i]}'
    ...
    def collect(self, job):
        ...
        round, path, node, _, _ = job
        atom_pos, atom_type, atom_symm, grid_name, space_group, lack = [], [], [], [], [], []
...
class Search(GeoCheck, PlanarSpaceGroup, Transfer):
    ...
    def save(self, atom_pos, type, symm, grid, sg, energy, path, node):
        """
...
if __name__ == '__main__':
    ...
    #Searching
    worker = Search(round)
    worker.explore(pos, type, symm, grid, sg, path, node)